...
for item in dis:
    ...
    if item["type"]==NEMATIC:
        plt.figure(1)
        plt.plot(item["data_r"], linewidth=3.0, color=colormap(normalize(item["eb"])), label=NEMATIC)  # linestyle="--",
    elif item["type"]==ANTINEMATIC:
        plt.figure(2)
        plt.plot(item["data_r"], linewidth=3.0, color=colormap(normalize(item["eb"])), label=ANTINEMATIC)  # linestyle="-",
